...
module tb_decoder;
  ...
  import cvxif_types_pkg::*;
    ...
    // logic                buffer_req_valid_i;
    // buff_dec_t           buffer_req_i;
    // hartid_t             hartid_i, hartid_o;
    // id_t                 id_i, id_o;
    x_issue_t     instr_req;
    logic         dec_resp_valid_o;
    vec_decoded_t decoded_req_o;
    ...
    assign instr_req.req.instr      = instr;
    assign instr_req.req.hartid     = 1'h1;;
    assign instr_req.req.id         = 5'hD;;
    assign instr_req.resp           = x_issue_resp;
    assign instr_req.register.rs[0] = curr_rs1;
    assign instr_req.register.rs[1] = curr_rs2; 
    ...
    initial begin : stim
        ...
        while (!$feof(file_handle)) begin
            ...
            total_instr++;
            if (x_issue_resp.accept) accepted_instr++;
            else         illegal_instr++;

            if (verbose || (x_issue_resp.accept != expected_accept) ||
                (x_issue_resp.accept && (vec_instr !== expected))) begin

                $display("[TB] instr #%0d  [0x%08h]  mnemonic=%-14s",
                        total_instr, curr_instr, curr_mnemonic);
                $display("         fields : %s", decode_fields(curr_instr));
                $display("         x_issue_resp.accept=%0b  x_issue_resp.writeback=%0b  reg_read={rs2=%0b,rs1=%0b}  vec_instr=%0s",
                        x_issue_resp.accept, x_issue_resp.writeback,
                        x_issue_resp.register_read[1], x_issue_resp.register_read[0],
                        vec_instr.name());

                // Check 1: accept vs expected
                if (x_issue_resp.accept !== expected_accept) begin
                $display("         MISMATCH  x_issue_resp.accept got=%0b expected=%0b",
                            x_issue_resp.accept, expected_accept);
                mismatch_instr++;
                end

                // Check 2: decoded enum vs expected (only when accepted)
                if (x_issue_resp.accept && expected_accept && (vec_instr !== expected)) begin
                $display("         MISMATCH  vec_instr got=%-20s expected=%s",
                            vec_instr.name(), expected.name());
                mismatch_instr++;
                end

                // Check 3: x_issue_resp.writeback sanity – only cfg instr should set it
                if (x_issue_resp.writeback && !(curr_mnemonic inside {"vsetvli","vsetivli","vsetvl"})) begin
                $display("         WARNING   x_issue_resp.writeback=1 but mnemonic is not a cfg instr");
                end

                // Check 4: x_issue_resp.register_read flags          
                expected_rr = expected_regread(curr_mnemonic);
                if (x_issue_resp.accept && (x_issue_resp.register_read !== expected_rr)) begin
                $display("         MISMATCH  x_issue_resp.register_read got=%02b expected=%02b  (rs2=%0b rs1=%0b)",
                            x_issue_resp.register_read, expected_rr,
                ...
                if (verbose && x_issue_resp.accept === expected_accept &&
                    (!x_issue_resp.accept || vec_instr === expected))
                $display("         OK");
    ...
    // assign buffer_req_valid_i                  = issue_ready;
    // assign buffer_req_i.instr_enum             = vec_instr;
    // assign buffer_req_i.instr                  = instr;
    // assign buffer_req_i.registers[0]           = curr_rs1;
    // assign buffer_req_i.registers[1]           = curr_rs2;
    // assign buffer_req_i.x_issue_resp.writeback = x_issue_resp.writeback;
    // assign hartid_i                            = 1'h1;
    // assign id_i                                = 5'hD;
