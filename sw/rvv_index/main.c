...
int main() {
  ...
  // TEST 8 (uint8, M)
  const uint8_t seed8 = 0xca;
  srand(seed8);

  uint8_t W[M], X[M];
  for (int i = 0; i < M; i++) { W[i] = rand(); X[i] = rand(); }

  uint8_t golden8[M], actual8[M];

  CSR_CLEAR_BITS(CSR_REG_MCOUNTINHIBIT, 0x1);
  CSR_WRITE(CSR_REG_MCYCLE, 0);
  index_golden8(golden8, W, X, M);
  CSR_READ(CSR_REG_MCYCLE, &cycles);
  printf("%d, ", cycles);

  CSR_CLEAR_BITS(CSR_REG_MCOUNTINHIBIT, 0x1);
  CSR_WRITE(CSR_REG_MCYCLE, 0);
  index_vec8(actual8, W, X, M);
  CSR_READ(CSR_REG_MCYCLE, &cycles);
  printf("%d \r\n", cycles);

  pass = 1;
  for (int i = 0; i < M; i++) {
    if (golden8[i] != actual8[i]) { pass = 0; break; }
  }
  puts(pass ? "pass 8" : "fail 8");


  // TEST 9 
  const uint8_t seed9 = 0x12;
  srand(seed9);

  uint8_t Y[O], Z[O];
  for (int i = 0; i < O; i++) { Y[i] = rand(); Z[i] = rand(); }

  uint8_t golden9[O], actual9[O];

  CSR_CLEAR_BITS(CSR_REG_MCOUNTINHIBIT, 0x1);
  CSR_WRITE(CSR_REG_MCYCLE, 0);
  index_golden8(golden9, Y, Z, O);
  CSR_READ(CSR_REG_MCYCLE, &cycles);
  printf("%d, ", cycles);

  CSR_CLEAR_BITS(CSR_REG_MCOUNTINHIBIT, 0x1);
  CSR_WRITE(CSR_REG_MCYCLE, 0);
  index_vec8(actual9, Y, Z, O);
  CSR_READ(CSR_REG_MCYCLE, &cycles);
  printf("%d \r\n", cycles);

  pass = 1;
  for (int i = 0; i < O; i++) {
    if (golden9[i] != actual9[i]) { pass = 0; break; }
  }
  puts(pass ? "pass 9" : "fail 9");
